...
var PageSetting = {
	...
	init:function(config){  // 初始化
		...
		if(this.downBtn === false && this.pullDownEl){
	...
	loaded:function  () {
			...
			this.myScroll.on('scroll', function(){
				if (PageSetting.downBtn && !PageSetting.downHolding && this.y > 3) {
			...
			this.myScroll.on('scrollEnd',  function(){
				if (PageSetting.downBtn && PageSetting.downHolding ) { // 下拉刷新第一页
